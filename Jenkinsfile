    ...
    stages {
        ...
        stage("Deploy") {
            steps {
                ...
                script {
                    sh """
                         ssh -i ${DEPLOY_SSH_KEY} ${DEPLOY_USERNAME}@${SERVER} "rm -rf /tmp/src"
                        scp -r -i ${DEPLOY_SSH_KEY} src  ${DEPLOY_USERNAME}@${SERVER}:/tmp/
                        ssh -i ${DEPLOY_SSH_KEY} ${DEPLOY_USERNAME}@${SERVER} "sudo cp -r /tmp/src/*  /var/www/html/"
